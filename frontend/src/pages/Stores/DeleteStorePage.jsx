...
const DeleteStorePage = () => {
  ...
  const handleDeleteStore = (storeToDelete) => {
    const confirmationText = storeToDelete.name;
    setDialogTitle(translations.confirmDeleteTitle || "Confirm Deletion");
    setDialogMessage(
      `${
        translations.confirmDeleteMessage || "Are you sure you want to delete"
      } '${confirmationText}'?`
    );
    setDialogConfirmationText(confirmationText);
  ...
  // FIX: Determine the stores to be filtered based on role
  const availableStores = useMemo(() => {
    if (isAdmin) return stores;
    if (isCountryChief)
      return stores.filter((store) => store.country === profileUser.country);
    return []; // Other roles may not see this page, so return empty
  }, [stores, profileUser, isAdmin, isCountryChief]);

  // FIX: Filter options and filtered list are now calculated over 'availableStores'
  const uniqueCountries = useMemo(() => {
    if (isAdmin)
      return [...new Set(availableStores.map((store) => store.country))].sort();
    if (isCountryChief) return [profileUser.country];
    return [];
  }, [availableStores, profileUser, isAdmin, isCountryChief]);

  const uniqueCities = useMemo(() => {
    let relevantStores =
      selectedCountries.length > 0
        ? stores.filter((store) => selectedCountries.includes(store.country))
        : stores;
    return [...new Set(relevantStores.map((store) => store.city))]
      ...
      .sort();
  }, [stores, selectedCountries, cityInput, selectedCities]);
  ...
  }, [stores, selectedCountries, selectedCities]);
  ...
  return (
    <div
      ...
      <h1 className="text-3xl font-semibold mb-6">
        {translations.deleteStoreTitle || "Delete Store"}
      </h1>
      <div className="flex flex-wrap items-start gap-4 mb-4">
        <div className="relative" ref={countryInputRef}>
          ...
          <ChevronDown
            size={18}
            className="absolute right-2 top-1/2 transform -translate-y-auto"
          />
          {showCountrySuggestions && uniqueCountries.length > 0 && (
            <ul
              ...
              {uniqueCountries.map((country) => (
                <li
                  key={country}
                  onClick={() => addFilter(country, "country")}
                ...
                <tr
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className="bg-red-500 hover:bg-red-600 text-white font-bold py-1.5 px-3 rounded-md">
